...
int FlashRequest_setDmaKtOutputRef ( struct PortalInternal *p, const uint32_t sgIdKtBuf0, const uint32_t sgIdKtBuf1, const uint32_t sgIdKtBuf2, const uint32_t sgIdKtBuf3, const uint32_t sgIdInvalPPA )
{
    volatile unsigned int* temp_working_addr_start = p->transport->mapchannelReq(p, CHAN_NUM_FlashRequest_setDmaKtOutputRef, 6);
    volatile unsigned int* temp_working_addr = temp_working_addr_start;
    if (p->transport->busywait(p, CHAN_NUM_FlashRequest_setDmaKtOutputRef, "FlashRequest_setDmaKtOutputRef")) return 1;
    p->transport->write(p, &temp_working_addr, sgIdKtBuf0);
    p->transport->write(p, &temp_working_addr, sgIdKtBuf1);
    p->transport->write(p, &temp_working_addr, sgIdKtBuf2);
    p->transport->write(p, &temp_working_addr, sgIdKtBuf3);
    p->transport->write(p, &temp_working_addr, sgIdInvalPPA);
    p->transport->send(p, temp_working_addr_start, (CHAN_NUM_FlashRequest_setDmaKtOutputRef << 16) | 6, -1);
    return 0;
};
...
{
    return "{\"setDebugVals\": [\"long\", \"long\"], \"writePage\": [\"long\", \"long\", \"long\", \"long\", \"long\", \"long\"], \"eraseBlock\": [\"long\", \"long\", \"long\", \"long\"], \"debugDumpReq\": [\"long\"], \"setDmaKtSearchRef\": [\"long\"], \"setDmaWriteRef\": [\"long\"], \"setDmaKtOutputRef\": [\"long\", \"long\", \"long\", \"long\", \"long\"], \"setDmaKtPpaRef\": [\"long\", \"long\", \"long\", \"long\"], \"findKey\": [\"long\", \"long\", \"long\"], \"startCompaction\": [\"long\", \"long\", \"long\"], \"setDmaReadRef\": [\"long\"], \"start\": [\"long\"], \"readPage\": [\"long\", \"long\", \"long\", \"long\", \"long\", \"long\"]}";
}
...
int FlashRequest_handleMessage(struct PortalInternal *p, unsigned int channel, int messageFd)
{
    ...
    case CHAN_NUM_FlashRequest_setDmaKtOutputRef: {
        ...
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.setDmaKtOutputRef.sgIdKtBuf0 = (uint32_t)(((tmp)&0xfffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.setDmaKtOutputRef.sgIdKtBuf1 = (uint32_t)(((tmp)&0xfffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.setDmaKtOutputRef.sgIdKtBuf2 = (uint32_t)(((tmp)&0xfffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.setDmaKtOutputRef.sgIdKtBuf3 = (uint32_t)(((tmp)&0xfffffffful));
        tmp = p->transport->read(p, &temp_working_addr);
        tempdata.setDmaKtOutputRef.sgIdInvalPPA = (uint32_t)(((tmp)&0xfffffffful));
        ((FlashRequestCb *)p->cb)->setDmaKtOutputRef(p, tempdata.setDmaKtOutputRef.sgIdKtBuf0, tempdata.setDmaKtOutputRef.sgIdKtBuf1, tempdata.setDmaKtOutputRef.sgIdKtBuf2, tempdata.setDmaKtOutputRef.sgIdKtBuf3, tempdata.setDmaKtOutputRef.sgIdInvalPPA);
      } break;
