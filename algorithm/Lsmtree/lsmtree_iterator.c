...
uint32_t __lsm_range_get(request *const req){ //after range_get
	...
	for_each_list_node(rgparams->algo_params_list, temp){
		...
		level_op_iterator_set_iterator(loi[al_params->lev], al_params->idx, al_params->copied_data, req->key, req->magic);
		free(al_params);
	}
	...
	for(uint32_t i=rgparams->total_loi_num-1; i>=0; i--){
		ka_pair t;
		while(level_op_iterator_pick_key_addr_pair(loi[i],&t)){
			skiplist_insert_iter(temp_list, t.key, t.ppa);
...
uint32_t lsm_range_get(request *const req){
	...
	if(ISTRANSACTION(LSM.setup_values)){
		for(uint32_t i=0; i<target_trans_entry_num; i++){
			params->loi[target_trans_entry_num-1-i]=level_op_iterator_transact_init(trans_sets[i], req->key, &ppa, req->magic, &should_read);
			if(should_read){
	...
	uint32_t *ppa_list;
	for(uint32_t i=LSM.LEVELN-1; i>=0; i--){
		params->loi[i+target_trans_entry_num]=level_op_iterator_init(LSM.disk[i], req->key, &ppa_list, req->length/(PAGESIZE-1024/DEFKEYLENGTH), req->magic, &should_read);
		if(should_read){
